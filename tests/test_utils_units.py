...
@pytest.mark.parametrize('units_in, units_out', [
    ...
    ('(centimeter^2)(meq/m^3 cm/s)', '(centimeter^2)(mmol/m^3 cm/s)'), # ALK_RIV_FLUX integral
...
def test_conv_units(apply_chunk, add_encoding):
    ...
    assert dict_skip_keys(da_out.attrs, skip_attr_list) \
        == dict_skip_keys(da.attrs, skip_attr_list)
