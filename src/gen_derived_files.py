...
def gen_derived_files(
    varname, component, experiment, stream=None, keep_read_src_files=True
):
    """
    ...
    for ensemble in range(len(expr_metadata)):
        ...
        hpss_dir = "/".join(
            [expr_metadata[ensemble]["hpss_root_dir"], component, "proc", "tseries"]
        )
        ...
        for src_varname in var_spec["derived_from_varnames"]:
            ...
            src_fnames_hpss_dict[src_varname] = [
                fname for fname in hsi_out if str_match in fname
            ]
        ...
        src_fnames_datestrs_0 = [
            fname.split(".")[-2] for fname in src_fnames_hpss_dict[src_varname_0]
        ]
        for src_varname in var_spec["derived_from_varnames"][1:]:
            src_fnames_datestrs = [
                fname.split(".")[-2] for fname in src_fnames_hpss_dict[src_varname]
            ]
            if src_fnames_datestrs != src_fnames_datestrs_0:
                print(
                    f"filename date-strings for {src_varname_0} do not match those for {src_varname}"
                )
        ...
        src_fnames_disk = [
            fname.replace(dir_old, dir_new)
            for fname in src_fnames_hpss_dict[src_varname_0]
        ]
...
def gen_file_z_reduce(z_reduce_op, src_fname_disk_dict, varname, dst_fname_disk):
    ...
    with xr.open_dataset(src_fname_disk) as ds_in:
        ...
        drop_coordnames = [
            ...
            if coordname not in ds_in[src_varname].encoding["coordinates"]
        ]
        ...
        vars_on_drop_coordnames = [
            ...
            and (
                set(drop_coordnames)
                & set(ds_in[varname_tmp].encoding["coordinates"].split(" "))
            )
        ]
        ...
        da_out = (
            ds_in.drop(drop_coordnames)[src_varname].isel({vert_dim: 0}).drop(vert_dim)
        )
        ...
        if z_reduce_op in ["zint_100m", "zint"]:
            ...
            zint_weight = (
                ds_in["dz"].isel(z_t=slice(0, vert_dim_len)).rename({"z_t": vert_dim})
            )
            sel_dict = {}
