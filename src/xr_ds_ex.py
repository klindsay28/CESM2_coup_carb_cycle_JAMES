...
def xr_ds_ex(decode_times=True, nyrs=3, var_const=True):
    ...
    time_var = xr.DataArray(time_values, name='time', dims='time', coords={'time':time_values},
                            attrs={'bounds':'time_bounds'})
    if not decode_times:
        time_var.attrs['units'] = time_units
        time_var.attrs['calendar'] = calendar
    time_bounds = xr.DataArray(time_bounds_values, name='time_bounds', dims=('time', 'd2'),
                               coords={'time':time_var})
    var = xr.DataArray(var_values, name='var_ex', dims='time', coords={'time':time_var})
    ds = var.to_dataset()
