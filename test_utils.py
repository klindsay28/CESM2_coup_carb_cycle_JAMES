...
def test_repl_coord(decode_times1, decode_times2, apply_chunk1):
    ...
    ds2 = repl_coord('time', ds1, xr_ds_ex(decode_times2, nyrs=nyrs, var_const=var_const))
...
def test_time_set_mid(decode_times, deep, apply_chunk):
    ...
    if decode_times:
        ...
        expected_values = cftime.num2date(mid_month_values,
                                          time_encoding['units'], time_encoding['calendar'])
    else:
...
def test_da_w_lags(decode_times, add_encoding_var):
    ...
    assert np.all(da2.isel(time=itime).values \
                  == da.isel(time=itime+np.array(lag_values)))
